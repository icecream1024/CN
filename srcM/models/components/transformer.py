...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):  # input_Q: [batch_size, len_q, d_model]
        ...
        context, attn = ScaledDotProductAttention()(Q, K, V,
                                                    attn_mask)  # context: [batch_size, n_heads, len_q, d_v]
...
for epoch in range(20):
    for enc_inputs, dec_inputs, dec_outputs in loader:  # enc_inputs : [batch_size, src_len]
        ...
        loss.backward()
        optimizer.step()
def main(model, enc_input, start_symbol):
...
predict_dec_input = main(model, enc_inputs[0].view(1, -1).cuda(), start_symbol=tgt_vocab["S"])
